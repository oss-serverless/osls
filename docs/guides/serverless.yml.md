...
functions:
  ...
  hello:
    ...
    tracing: Active
    # Conditionally deploy the function and the companion resources generated by the
    # core functions compiler (version, alias, function URL, event invoke config) and
    # the function's log group. Define the condition under resources.Conditions.
    # When the condition evaluates to false on an update, CloudFormation deletes the
    # function's log group INCLUDING all retained log events. This matches the
    # behavior of removing the function from the configuration.
    # To preserve logs across condition flips, override the generated log group:
    # resources:
    #   Resources:
    #     HelloLogGroup:
    #       DeletionPolicy: Retain
    # A retained physical log group must be manually deleted or imported before the
    # condition can flip back to true.
    # Resources generated for `events` (Lambda permissions, subscriptions, rules,
    # API Gateway resources, etc.) do NOT inherit this condition, so combining
    # `condition` with `events` can fail to deploy when the condition is false.

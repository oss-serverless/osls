...
class AwsProvider {
  constructor(serverless, options) {
    ...
    this.limitEcrRequests = promiseLimit(6, async (task) => task());
    ...
    this.hooks = {
      initialize: () => {
        ...
        if (provider && provider.deploymentBucket) {
          if (isObject(provider.deploymentBucket)) {
        ...
        function: {
          properties: {
            ...
            durableConfig: {
              type: 'object',
              properties: {
                executionTimeout: { type: 'integer', minimum: 1, maximum: 31622400 },
            ...
            kmsKeyArn: { $ref: '#/definitions/awsKmsArn' },
            recursiveLoop: { enum: ['Allow', 'Terminate'] },
            ...
            vpc: {
              // `false` and `null` opt the function out of `provider.vpc`
              anyOf: [
    ...
    this.awsSdkV3CredentialsProviders = new Map();

    // Core no longer reads it, but it is kept as some plugins
    // may rely on `provider.accountId`
    this.accountId = null;
  ...
  async getAwsSdkV3Config(options = {}) {
    const { credentials, profile, region, ...clientOptions } = options;
    delete clientOptions.service;

    return buildClientConfig({
      ...clientOptions,
      region: region === undefined ? this.getRegion() : region,
      credentials: credentials || this.getAwsSdkV3CredentialsProvider({ profile }),
    });
  }

  getCloudFormationClient() {
    this.cloudFormationClientPromise ||= this.getAwsSdkV3Config().then(
  ...
  getAwsSdkV3CredentialsProvider(options = {}) {
    ...
    if (!this.awsSdkV3CredentialsProviders.has(cacheKey)) {
      const credentialsProvider = getAwsSdkV3CredentialsProvider({ provider: this, ...options });
      // Memoize process-wide; the memoized flag stops each client adding its own memoizer
      // and re-resolving (repeated MFA prompts, AssumeRole, credential_process). The error
      // mapping sits inside the memoized function so it applies on every resolution.
  ...
  getValues(source, objectPaths) {
    return objectPaths.map((objectPath) => ({
      path: objectPath,
      value: getByPath(source, objectPath),
    }));
  }
  ...
  getRuntime(runtime) {
    const defaultRuntime = 'nodejs24.x';
  ...
  isExistingRoleProvided(role) {
    return (
      typeof role === 'string' ||
      (isObject(role) && Object.keys(role).some((key) => key.includes('::')))
    );
  }
  ...
  getCustomExecutionRole(functionObj) {
    ...
    if (hasOwn(functionObj, 'role') && functionObj.role) return functionObj.role;

    const role =
      hasOwn(provider, 'iam') && isObject(provider.iam) && hasOwn(provider.iam, 'role')
        ? provider.iam.role
        : undefined;
  ...
  resolveFunctionIamRoleResourceName(functionObj) {
    ...
    if (customRole) {
      ...
      if (
        ...
        hasOwn(customRole, 'Fn::ImportValue') ||
        hasOwn(customRole, 'Ref') ||
        hasOwn(customRole, 'Fn::Sub')
      ) {
        return null;
      }
    }
  ...
  getStageSourceValue() {
    ...
    for (const [source, key] of sources) {
      if (hasOwn(source, key)) return { value: source[key] };
    }
  ...
  async getStackResources(next, resourcesParam) {
    ...
    do {
      ...
      const res = await cloudFormation.send(new ListStackResourcesCommand(params));
...
Object.defineProperties(
  ...
  memoizeeMethods({
    getAccountInfo: methodDescriptor(
      async function () {
        const sts = new STSClient(await this.getAwsSdkV3Config());
    ...
    ensureDockerIsAvailable: methodDescriptor(
      async () => {
        try {
    ...
    dockerLoginToEcr: methodDescriptor(
      async function () {
        const registryId = await this.getAccountId();
        const ecr = new ECRClient(await this.getAwsSdkV3Config());
        ...
        // Guard the spawn below: with no input to write, `docker login --password-stdin`
        // would block awaiting stdin EOF instead of failing.
        if (!decodedAuthToken) {
          throw new ServerlessError(
            'Unexpected ECR authorization token format',
            'DOCKER_LOGIN_ERROR'
          );
        }
        const dockerArgs = ['login', '--username', 'AWS', '--password-stdin', proxyEndpoint];
        try {
          const { stdBuffer } = await spawnExt('docker', dockerArgs, { input: decodedAuthToken });
    ...
    getOrCreateEcrRepository: methodDescriptor(
      async function (scanOnPush, maxImageCount) {
        ...
        try {
          ...
          if (!isEcrRepositoryNotFoundError(err)) throw err;
          const result = await ecr.send(
    ...
    resolveImageUriAndShaFromPath: methodDescriptor(
      async function ({
        imageName,
        imagePath,
        imageFilename,
    ...
    resolveImageUriAndShaFromUri: methodDescriptor(
      async function (image) {
        ...
        if (!this.ecrDescribeImagesClientPromise) {
          this.ecrDescribeImagesClientPromise = this.getAwsSdkV3Config().then(
            (config) => new ECRClient(config)
          );
        }

        const ecr = await this.ecrDescribeImagesClientPromise;
        const describeImagesResponse = await this.limitEcrRequests(() =>
          ecr.send(
    ...
    resolveImageUriAndSha: methodDescriptor(
      async function (functionName) {
        ...
        const providerEcrConfig = this.serverless.service.provider.ecr || {};
        const imageDefinedInProvider =
          providerEcrConfig.images && providerEcrConfig.images[imageName];

        const imageScanDefinedInProvider =
          providerEcrConfig.scanOnPush === undefined
            ? defaultScanOnPush
            : providerEcrConfig.scanOnPush;

        const maxImageCountProvider =
          providerEcrConfig.maxImageCount === undefined
            ? defaultMaxImageCount
            : providerEcrConfig.maxImageCount;
        ...
        if (isObject(imageDefinedInProvider)) {
